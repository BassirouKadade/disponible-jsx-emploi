...
export default function ListeModulesDis() {
  // État pour sker la liste des salles
  const {idFormateur}=useParams()
...
useEffect(() => {
  ...
  const getData = async () => {
    try {
      ...
      const reponse = await fetch(`http://localhost:3001/listeModulesFormateurs?idFormateur=${idFormateur}`);
...
useEffect(() => {
  ...
  const getData = async () => {
    try {
      ...
      const reponse = await fetch(`http://localhost:3001/FormateurIdFormateurParamere?idFormateur=${idFormateur}`);

      if (reponse.ok) {
        const data = await reponse.json();
       
        setInfo(prevState=>({...prevState,nom:data[0].nom,prenom:data[0].prenom}))
      } else {
        console.log('Une erreur est survenue...');
      }
    } catch (e) {
...
async function suppressionLigne(idSalle) {
  ...
  if (ok) {
    try {
      ...
      const reponse = await fetch(`http://localhost:3001/supressionAssociationModule?iddAssociationModule=${idSalle}`);
  ...
  loadings.loadingSalle || actions.copieListetTri.length === 0 && !demarrage ?
    ...
    <section className={`sectionListe  ${showFormMod ? "schedules" : ""}`}>
      ...
      <div  style={{marginTop:"10px"}}>
            ...
            <h3 style={{ textAlign:"left",fontStyle:'italic',fontSize:"21px"}} id='Text'>Mr {info.nom} {info.prenom}  </h3>
               <Link to={'/formateurs/moduleDispense'} className='AjouterAuyeModule' ><FontAwesomeIcon icon={faPlusCircle} /> <span style={{margin:"0 2px"}}>Ajouter un autre module</span>    </Link>
             </div>
           <hr />
      </div>
