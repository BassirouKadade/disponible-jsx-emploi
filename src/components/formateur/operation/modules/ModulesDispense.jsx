...
export default function ModulesDispense() {
  ...
  useEffect(() => {
    ...
    const fetchDataM = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3001/getModulesId?idFormateur=${!groupeData.formateur?0:groupeData.formateur}`);
        if (response.ok) {
...
useEffect(()=>{
  ...
  const fetchDataF = async () => {
    try {
      ...
      setLoadingData(true);
      const response = await fetch('http://localhost:3001/getFormateurs');
      if (response.ok) {
        const data = await response.json();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3001/ajoutAssociationFormMod', {
        method: 'POST',
        headers: {
      ...
      <AnimComponent padParent={20} padChild={10} borderColor={'rgba(29, 120, 120, 0.843)'} bord={3} />
        ...
        <>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between", marginTop:"10px"}} className="salle">
            <h3 id='salleText'>Les modules de formateur  </h3>
            {groupeData.formateur&& <span style={{color:"rgba(29, 120, 120, 0.843)"}}>
            <Link to={`/formateurs/listeDesmodulesDeformaeur/${groupeData.formateur}`} className='AjouterAuyeModule' ><FontAwesomeIcon icon={faEye} />  <span style={{margin:"0 2px"}}>voir ses modules</span> </Link>
            </span>}
          ...
          <form action="" method="post" className="module" onSubmit={handleSubmit}>
            <div className="salleEnfant">
                <div className="info">
                <label htmlFor="formateur">Formateur <span className='champsO'>*</span></label>
                <select
                  id="formateur"
                  name="formateur"
                ...
                {error.formateur && <div className="invalid-feedback">le formateur ne doit pas etre vide</div>}  
              </div>
              <div className="info">
                ...
                >
                  ...
                  { listeModule.length===0? <option value="">Le formateur à tous les modules disponibles . verifier ses modules</option>:
