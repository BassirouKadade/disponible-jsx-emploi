...
import { faMagnifyingGlass,faXmark,faChevronLeft,faChevronRight,faCheck,faFaceRollingEyes,faTrash ,faQuestionCircle,faEye,faSync,faFilePen,} from '@fortawesome/free-solid-svg-icons';
...
useEffect(() => {
  ...
  const getData = async () => {
    try {
      ...
      const reponse = await fetch('http://localhost:3001/listeFormateurs');
...
function tri(e) {
  ...
  setSalles((prevState) => {
    ...
    switch (e.target.value) {
      ...
      case "nom":
        sortedSalles.sort((a, b) => a.nom.localeCompare(b.nom));
        break;

        case "prenom":
          sortedSalles.sort((a, b) => a.prenom.localeCompare(b.prenom));
          break;
      case "email":
            sortedSalles.sort((a, b) => a.email.localeCompare(b.email));
            break;
      case "telephone":
              sortedSalles.sort((a, b) => a.telephone.localeCompare(b.telephone));
              break;
  ...
  try {
    ...
    if (actions.elementRecherche !== "") {
      const nouvelle = actions.copieListetTri.filter((element) =>
        element.nom.toString().toLowerCase().includes(actions.elementRecherche.toString().toLowerCase()) ||
        element.prenom.toString().toLowerCase().includes(actions.elementRecherche.toString().toLowerCase()) ||
        element.email.toString().toLowerCase().includes(actions.elementRecherche.toString().toLowerCase())||
        element.telephone.toString().toLowerCase().includes(actions.elementRecherche.toString().toLowerCase())
      );
      setSalles(nouvelle);
    } else {
      return;
...
async function suppressionLigne(idSalle) {
  ...
  if (ok) {
    try {
      ...
      const reponse = await fetch(`http://localhost:3001/supressioSFormateur?idFormateur=${idSalle}`);
...
async function modifcationElementBaseDedonees(e) {
  ...
  try {
    ...
    const reponse = await fetch('http://localhost:3001/ModicationFormateurs', {
      method: "PUT",
      headers: {
          ...
          <form action="" onSubmit={modifcationElementBaseDedonees}>
            <div className="salleEnfantmod">
              ...
              <div className="infoMod">
                ...
                <input
                  type="text"
                  id="nom"
                  onChange={(e) => setModification(prevState => ({ ...prevState, nom: e.target.value }))}
                  name="nom"
                  placeholder="Nom du formateur"
                  value={modification.nom}
              ...
              <div className="infoMod">
              <label htmlFor="nom">Prénom <span className='champsO'>*</span></label>
              <input
                type="text"
                id="prenom"
                onChange={(e) => setModification(prevState => ({ ...prevState,prenom: e.target.value }))}
                name="prenom"
                placeholder="Prenom du formateur"
                value={modification.prenom}
            ...
            <div className="salleEnfantmod">
              ...
              <input
                type="email"
                id="email"
                onChange={(e) => setModification(prevState => ({ ...prevState,email: e.target.value }))}
                name="email"
              ...
              <input
                type="text"
                id="telephone"
                onChange={(e) => setModification(prevState => ({ ...prevState,telephone: e.target.value }))}
                name="telephone"
                placeholder="Télephone du formateur"
                value={modification.telephone}
          ...
          {!loadSearch ?
            salles.length > 0 ? salles.map((salle, index) => (
              <tr key={index}>
                ...
                <td>{salle.prenom}</td>
                <td>{salle.email}</td>
                <td>{salle.telephone.length===0?"Aucun numéro":salle.telephone}</td>
                <td> 
                <Link  to={`/formateurs/listeDesmodulesDeformaeur/${salle.idFormateur}`} className='hoverNone' ><FontAwesomeIcon icon={faEye} />  <span style={{margin:"0 2px"}}>voir ses modules</span> </Link>
                </td>
                <td style={{ display: "flex" }}>
                  ...
                  <button style={{ width: "32px", position: "relative" }} className='button-supprimer'>
                    {!loadings.loadingSuppression ? (
                      <span onClick={() => suppressionLigne(salle.idFormateur)} className='supStyle'>
